...
Étapes de mise en place d'un `RecyclerView` qui gère une liste de `String`.

<Tabs>
  <TabItem value="build.gradle.kts" label="build.gradle.kts">

    Vous allez utiliser le [ViewBinding](https://developer.android.com/topic/libraries/view-binding#kts) pour la suite des choses. Assurez vous qu'il est activé dans votre fichier `build.gradle.kts`.
  ...
  <TabItem value="MonAdapter.kt" label="MonAdapter.kt" default>
    ...
    Vous allez créer une classe qui est responsable de la création de **TOUT** les éléments de la liste et de leur mise à jour.
    ...
    class MonAdapter
      ...
      inner class AdapterItemViewHolder(private val binding: MonItemBinding) : RecyclerView.ViewHolder(binding.root) {
          fun bind(adapterItem: String) {
              binding.tvNoTirage.text = adapterItem
          }
      }
    ...
    Une classe interne (`inner class`) est une classes qui ne peut être utilisée que par sa classe parente. 
    Dans votre cas, la classe `AdapterItemViewHolder` ne pourra être utiliée que par sa classe parente, soit `MonAdapter`.
    Nous aurions pu la créer en dehors de la classe `MonAdapter`, dans son propre fichier, mais c'est surtout par soucis de clarté que nous la définissons à l'intérieur puisqu'elle ne sera jamais utilisée en dehors du contexte de la classe `MonAdapter`.

    La classe `AdapterItemViewHolder` prend en paramètre un objet de type `MonItemBinding`. 
    Cet objet est généré automatiquement puisque nous avons activé le `ViewBinding` et que nous avons créé un fichir `mon_item.xml` plus tôt.
    Il nous donne accès à tous les éléments de notre fichier `xml` sous forme de propriétés.

    La classe `AdapterItemViewHolder` hérite de la classe `RecyclerView.ViewHolder` qui est responsable de faire le lien avec le système de gestion de liste d'Android.
    Cette classe prend en paramètre le `root` de notre fichier `mon_item.xml` qui est l'élément le plus "haut" de notre fichier. 
    Dans le cas du fichier `mon_item.xml`, il s'agit de `LinearLayout`.

    La classe `AdapterItemViewHolder` définit une méthode `bind` qui prend en paramètre ce qu'on désire afficher dedans, soit une `String`.
    Cette méthode est responsable de mettre à jour les éléments de l'interface graphique de l'élément de la liste.
